 ...
 */
<#if swagger2>
    @Api(value = "${table.comment?substring(0,table.comment?length-1)}")
<#else>
</#if>
<#if restControllerStyle>
@RestController
...
@PostMapping("/page")
public Result
<IPage<${entity}>> page${entity}(@RequestBody ${entity} ${table.entityPath}) {
...
@DeleteMapping("/{ids}")
public Result<?> delete${entity}(@PathVariable("ids") List
<Long> ids) {
